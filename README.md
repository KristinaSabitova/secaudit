...
## domainaudit — domain & subdomain auditing

`secaudit.py` audits **source code**. `domainaudit.py` is a standalone tool
that audits your **live domains and subdomains**: the attack surface the
internet sees, without touching the code.

Ideal for self-auditing your companies' domains. **Only run it against domains
you own or have explicit permission to test.**

### Zero dependencies

Python 3.9+ standard library only. DNS lookups use DNS-over-HTTPS (Google /
Cloudflare), so it runs on any machine with HTTPS egress — no `dnspython`, no
local resolver, no external tools.

### What it detects

| Category | Checks |
|----------|--------|
| **Subdomains** | Enumeration via Certificate Transparency (crt.sh) + optional built-in wordlist brute force |
| **DNS** | DNSSEC, CAA records, NS |
| **TLS/certificates** | Expiry, invalid chain/hostname, obsolete protocols (TLS 1.0/1.1) |
| **HTTP headers** | HSTS, CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, HTTP→HTTPS redirect, insecure cookies |
| **Email** | SPF (incl. permissive `+all`), DMARC (incl. `p=none`), DKIM, MX |
| **Subdomain takeover** | Dangling CNAMEs pointing to unclaimed services (GitHub Pages, Heroku, S3, Azure…) |
| **Exposure** | Headers leaking technology/version (`Server`, `X-Powered-By`…) |
| **Ports** | Common-port scan; flags databases and services that shouldn't be exposed |

### Usage

```bash
# Passive audit of the apex domain (fast, non-intrusive)
python3 domainaudit.py mycompany.com

# Enumerate and audit subdomains (Certificate Transparency)
python3 domainaudit.py mycompany.com --subdomains

# Full audit: subdomains + brute force + port scan
python3 domainaudit.py mycompany.com --full --yes

# Multiple domains at once
python3 domainaudit.py company-a.com company-b.com --subdomains

# Output formats
python3 domainaudit.py mycompany.com --json -o report.json
python3 domainaudit.py mycompany.com --md   -o report.md
```

Passive checks (DNS, TLS, headers, email, CT subdomains) need no confirmation.
**Active** checks (`--ports`, `--brute`) prompt for ownership confirmation
unless you pass `--yes`.

### Reading the output

- Each finding carries a **severity**, **detail**, and a concrete **fix**.
- An aggregate **risk score 0–100** is computed per domain.
- Exit code `3` when any `critical`/`high` finding is present (handy for CI/cron).
